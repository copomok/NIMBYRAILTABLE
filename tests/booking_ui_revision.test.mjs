...
assert.ok(source.includes('_bookSeatHydrationGeneration'),'이전 검색의 대기 중 재고 계산을 취소하는 세대 검사가 누락되었습니다.');
assert.ok(source.includes('queue.shift()'),'여러 편성의 재고 계산을 한 프레임에 몰아서 실행하면 안 됩니다.');
