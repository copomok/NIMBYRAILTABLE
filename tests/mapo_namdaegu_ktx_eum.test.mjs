...
const down=['마포','서울','병목안','수영','오산','죽산','일죽','장호원','돈산','충주','수안보','북문경','문경','상주','청리','옥산','김천','구미','약목','서왜관','하빈','호림','남대구'];
const commercial=new Set(['마포','서울','병목안','수영','충주','문경','상주','구미','약목','서왜관','남대구']);

test('501~524는 12왕복 KTX-이음이며 기존 불완전 525~529는 대치된다',()=>{
  assert.equal(trains.length,24);
  assert.deepEqual(Array.from(trains,t=>Number(t.no)).sort((a,b)=>a-b),Array.from({length:24},(_,i)=>501+i));
...
test('양방향 80분 간격·첫막차 조건과 154분 소요시간을 지킨다',()=>{
  for(const dir of ['down','up']){
    ...
    assert.equal(list.at(-1).stops.at(-1).arr,'22:49');
    for(const train of list)assert.equal(elapsed(train.stops[0].dep,train.stops.at(-1).arr),154);
  }
});

test('4편성 운용은 전 편을 한 번씩 포함하고 6분 회차 후 원 기점으로 복귀한다',()=>{
  const source=fs.readFileSync(new URL('../js/nimbi_rail.js',import.meta.url),'utf8');
  const seqs=[
    ['501','506','509','514','517','522'],['503','508','511','516','519','524'],
    ['502','505','510','513','518','521'],['504','507','512','515','520','523']
  ];
  const byNo=new Map(trains.map(t=>[t.no,t]));
  assert.deepEqual(seqs.flat().map(Number).sort((a,b)=>a-b),Array.from({length:24},(_,i)=>501+i));
  for(const seq of seqs){
    ...
    for(let i=1;i<seq.length;i++){
      ...
      assert.equal(elapsed(previous.stops.at(-1).arr,next.stops[0].dep),6);
    }
  }
});
