...
for (const file of [
  'data/nimbi_rail_data.js', 'js/core/nimbi_rail_index.js', 'data/nimbi_station_data.js', 'data/nimbi_metro.js',
  'data/nimbi_metro_sched.js', 'data/nimbi_platform_db.js', 'data/nimbi_realplat.js',
  'data/nimbi_homonyms.js'
]) vm.runInContext(fs.readFileSync(file, 'utf8'), context, { filename:file });
vm.runInContext('this.result={trains:ALL_TRAINS,lines:METRO_LINES,schedules:METRO_SCHED,stations:STATION_DB,platforms:PLATFORM_DB,byStation:TRAINS_BY_STATION}', context);
...
test('대표 동명이역 쌍은 서로 다른 인게임 좌표를 유지한다', () => {
  const pairs = [
    ['장흥역','장흥(양주)역'], ['송정역','송정(부산)역'],
    ['춘양(전남)역','춘양역'], ['장성역','장성(포항)역'],
...
test('노선 문맥에 맞는 동명이역으로 연결된다', () => {
  ...
  assert.ok(context.result.trains.some(train => train.stops.some(stop => stop.s === '장흥(양주)')));
